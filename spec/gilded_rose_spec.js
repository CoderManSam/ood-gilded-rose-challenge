var {Shop, Item, Uncommon, Cheese, Legendary, BackstagePass, Conjured} = require('../src/gilded_rose.js');

describe("Gilded Rose", function() {

  it("item sellIn and quality degrades over time", function() {
    const gildedRose = new Shop([ new Uncommon("+5 Dexterity Vest", 10, 20) ]);
  ...
  it("multiple items sellIn and quality degrades over time", function() {
    const gildedRose = new Shop([ new Uncommon("+5 Dexterity Vest", 10, 20), new Uncommon("Elixir of the Mongoose", 5, 7) ]);
    const items = gildedRose.updateQuality();
    const result = []  
  ...
  it("past sell by date quality degrades twice as fast", function() {
    const gildedRose = new Shop([ new Uncommon("+5 Dexterity Vest", 0, 10) ]);
    const items = gildedRose.updateQuality();
    const result = [`${items[0].name}, ${items[0].sellIn}, ${items[0].quality}`]
  ...
  it("item quality doesn't drop below 0", function() {
    const gildedRose = new Shop([ new Uncommon("+5 Dexterity Vest", 0, 0) ]);
    const items = gildedRose.updateQuality();
    const result = [`${items[0].name}, ${items[0].sellIn}, ${items[0].quality}`]
  ...
  it("backstage pass quality increases by 2 when there are 10 days or less before sell by date", function() {
    const gildedRose = new Shop([ new BackstagePass("Backstage passes to a TAFKAL80ETC concert", 10, 45) ]);
    const items = gildedRose.updateQuality();
    const result = [`${items[0].name}, ${items[0].sellIn}, ${items[0].quality}`]
  ...
  it("backstage pass quality becomes 0 after sell by date", function() {
    const gildedRose = new Shop([ new BackstagePass("Backstage passes to a TAFKAL80ETC concert", 0, 45) ]);
    const items = gildedRose.updateQuality();
    const result = [`${items[0].name}, ${items[0].sellIn}, ${items[0].quality}`]
  ...
  it("conjured items degrade in quality twice as fast", function() {
    ...
    const items = gildedRose.updateQuality();
    const result = [`${items[0].name}, ${items[0].sellIn}, ${items[0].quality}`]
    expect(result).toEqual([("Conjured Mana Cake, 2, 4")]);
  });
